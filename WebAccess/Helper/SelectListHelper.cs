...
namespace WebAccess.Helper
{
    public class SelectListHelper
    {
        ...
        public static SelectList GetStoreEmployees(int storeid, object selectedValue = null)
        {
            ...
            var employee = context.employees.Where(x => x.userid == storeid).Where(x=>x.active == 1).Select(x => new {employeeid = x.employeeid, name = x.firstname + " " + x.lastname }).OrderBy(x=>x.name).ToList();
