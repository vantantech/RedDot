...
namespace RedDot
{
    public class DBCustomer
    {
        ...
        /// <returns></returns>
        public DataTable GetPurchaseHistory(int id, decimal rewardpercent,bool includeopen)
        {
            string query;



            query = "select sales.id,saledate, closedate, total, ((total - total * coalesce(rewardexception,0)) * " + rewardpercent + ") as rewardearned,  reward.amount as rewardused, status " +
                " from sales  " +
                " left outer join (select * from payment where payment.description='Reward') as reward on sales.id = reward.salesid " +
                " where sales.CustomerID = " + id + " and sales.status !='Open' ";

            if(includeopen) query = query +  " union select sales.id, saledate,closedate, total, 0 as rewardearned,  0 as rewardused , status" +
                                            " from sales  " +
                                             " where sales.CustomerID = " + id + " and sales.status='Open' ";
