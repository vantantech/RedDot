    ...
    public class DBAppointment
    {
        ...
        public bool CheckLength( DateTime apptdate, int employeeid, int length)
       {
        ...
        public bool SpaceAvailable(int currentapptid, DateTime apptdate, int employeeid, int length)
        {
         ...
         public int CreateNewAppointment(DateTime apptdate, int employeeid, int customerid, int length, string note, int catid)
        {
            string query;
            query = "Insert into appointment (apptdate,employeeid,customerid,length,note,catid) values ('" + apptdate.ToString("yyyy-MM-dd HH:mm") + "'," + employeeid + "," + customerid + "," + length + ",'" + note +  "'," + catid + ")";
        ...
        public DataTable GetAppointmentsByEmployee(int employeeid, DateTime appointmentdate)
       {
        ...
        {
            string query;
            query = "select appointment.*, category.colorcode as catcolorcode from appointment left outer join category on appointment.catid = category.id where  DATE(apptdate) ='" + appointmentdate.ToString("yyyy-MM-dd") + "' order by apptdate";
            return m_dbconnect.GetData(query);
        }
        ...
        public DataRow GetAppointmentByID(int id)
        {
            ...
            query = "select appointment.*, category.colorcode as catcolorcode from appointment left outer join category on appointment.catid = category.id where appointment.id=" + id;
