        ...
        public ActionResult Index(int? reporttype, int? StoreId, string startdate, string enddate)
        {
            ...
            if (defaultstore != null)
            {
              ...
              /*  var sales = (from s in context.sales
                             ...
                             group new { s, t, e } by new { s.id, s.saledate, c.firstname, c.lastname, s.ticketno, s.total, s.subtotal, s.status, s.note, s.adjustment, s.custom1, s.custom2, s.custom3, s.custom4, s.payments, s.gratuities } into g
                             select new Ticket
                             {
                                 id = g.Key.id,
                                 ...
                                 payments = g.Key.payments,
                                 gratuities = g.Key.gratuities,
                                 TicketItems = g.Select(x => new TicketItem {EmployeeName = x.e.firstname,quantity = x.t.quantity,description = x.t.description, price = x.t.price, discount = x.t.discount, note = x.t.note })
