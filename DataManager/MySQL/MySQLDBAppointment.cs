...
{
    public class MYSQLDBAppointment
    {
        DBConnect m_dbconnect;
        public MYSQLDBAppointment()
        {
       ...
       public bool UpdateString(int id, string field, string fieldvalue)
       {
           ...
           query = "update appointment set " + field + " ='" + fieldvalue.Replace("'","''") + "' where id=" + id;
           return m_dbconnect.Command(query);
        ...
        public bool CheckLength( DateTime apptdate, int employeeid, int length)
       {
        ...
        {
            string query;
            query = "Insert into appointment (apptdate,employeeid,customerid,length,note) values ('" + apptdate.ToString("yyyy-MM-dd HH:mm") + "'," + employeeid + "," + customerid + "," + length + ",'" + note +  "')";
        ...
        public DataTable GetAppointments(int employeeid, DateTime appointmentdate)
       {
        ...
        {
            string query;
            query = "select * from appointment  where  DATE(apptdate) ='" + appointmentdate.ToString("yyyy-MM-dd") + "' order by apptdate";
            return m_dbconnect.GetData(query);
        }
        ...
        public DataTable GetApptCategories(int apptid)
        {
            ...
            sql = "select category.*, appt2cat.id as selid from category " +
                 " inner join appt2cat on appt2cat.catid = category.id " +
                " where appt2cat.apptid=" + apptid ;
        ...
        {
            string sql = "insert into appt2cat (apptid,catid) values (" + apptid + "," + catid + ")";
            m_dbconnect.Command(sql);
        }

        public void RemoveApptCategory( int catid)
        {
            string sql = "delete from appt2cat where id =" + catid ;
            m_dbconnect.Command(sql);
        }
    }
}
