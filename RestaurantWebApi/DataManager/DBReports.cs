...
namespace RedDot
{
	public class DBReports
	{
		...
		public DataTable GetDailySales(DateTime reportdate)
		{
			...
			string query = "select (adjustment + itemdiscount) as discount, salesresult.*, (itemprice + coalesce(salesmodifierprice,0))  as itemtotal,paymentresult.tips, paymentresult.netpayment,( salesresult.total - paymentresult.netpayment) as balance from (select sales.id, sales.saledate, (sales.adjustment + sales.cashdiscount) as adjustment, sales.total, sales.subtotal,   (sum(salesitemcombined.discount * salesitemcombined.quantity) * (-1)) as itemdiscount, sum(salesitemcombined.price * salesitemcombined.quantity) as  itemprice , sum(salesmodifierprice * salesitemcombined.quantity) as salesmodifierprice,sales.salestax from sales " +
			" inner join (select salesitem.*,sum( salesmodifiers.price) as salesmodifierprice from salesitem  left outer join salesmodifiers on salesitem.id = salesmodifiers.salesitemid where salesitem.void =0 group by salesitem.id) as salesitemcombined on sales.id = salesitemcombined.salesid  " +
        ...
        public DataTable GetSalesByOrderType(DateTime startdate, DateTime enddate)
        {
            ...
            string sum = "select  count(*) as ticketcount, sum(total)/count(*) as avgticketamount from sales  where total > 0 and  DATE(sales.saledate) between '" + startdate.ToString("yyyy-MM-dd") + "' and '" + enddate.ToString("yyyy-MM-dd") + "'";
        ...
        public DataTable GetTicketPaymentRefunds(DateTime startdate, DateTime enddate)
        {
            ...
            string query = "select ordertype,sales.id,saledate,paymentdate, concat( payment.cardgroup,payment.cardtype) as description, (netamount + tipamount) as total, payment.custom4 from sales " +
                            " inner join payment on sales.id = payment.salesid " +
                            " where (transtype='RETURN' or transtype='REFUND') and  DATE(paymentdate) between '" + startdate.ToString("yyyy-MM-dd") + "' and '" + enddate.ToString("yyyy-MM-dd") + "' ";
            return m_dbconnect.GetData(query);
        }
        ...
        public DataTable GetTicketDiscounts(DateTime startdate, DateTime enddate)
        {
            ...
            string query = "select * from sales where adjustment != 0 and  DATE(saledate) between '" + startdate.ToString("yyyy-MM-dd") + "' and '" + enddate.ToString("yyyy-MM-dd") + "' ";
            return m_dbconnect.GetData(query);
        }
        ...
        public DataTable GetTicketItemDiscounts(DateTime startdate, DateTime enddate)
        {
            ...
            string query = "select * from sales inner join salesitem on sales.id = salesitem.salesid where discount != 0 and  DATE(saledate) between '" + startdate.ToString("yyyy-MM-dd") + "' and '" + enddate.ToString("yyyy-MM-dd") + "' ";
            return m_dbconnect.GetData(query);
        }
        ...
        public decimal GetCashPaymentTotal(int employeeid, string cardgroup, DateTime startdate, DateTime enddate)
        {

            string query = "select sum(netamount) as total from payment inner join sales on sales.id = payment.salesid where sales.employeeid=" + employeeid + " and  UPPER(cardgroup) = '" + cardgroup.Trim().ToUpper() + "' and date(paymentdate) between '" + startdate.ToString("yyyy-MM-dd") + "' and '" + enddate.ToString("yyyy-MM-dd") + "'";
        ...
        public DataTable GetSalesRevenue(DateTime startdate, DateTime enddate)
		{
            ...
            string query = "select sum(coalesce(cnt,0)) as qty,  reportgroup.reportgroup, sum(netrevenue) as netrevenue, sum(grossrevenue) as grossrevenue from reportgroup " +
                           ...
                           " select sum(coalesce(salesitem.quantity * salesitem.weight,0)) as cnt, salesitem.reportcategory,sum((salesitem.price * salesitem.weight + salesitem.modprice - salesitem.discount) * salesitem.quantity) as netrevenue, " +
                            ...
                            " from sales inner join (select salesitem.*, coalesce(sum(coalesce(salesmodifiers.price, 0) * salesmodifiers.quantity),0) as modprice from salesitem " +
                            " left outer join salesmodifiers on salesitem.id = salesmodifiers.salesitemid where salesitem.void = 0 and salesitem.type != 'combo' " +
                            ...
                            " inner join ( select weight, combo.salesid,combo.id,price,discount,quantity,reportcategory,modprice , sum(diff) as diff from(select salesitem.*, coalesce(sum(coalesce(salesmodifiers.price, 0) * salesmodifiers.quantity),0) as modprice from salesitem " +
                            " left outer join salesmodifiers on salesitem.id = salesmodifiers.salesitemid where salesitem.void = 0 and salesitem.type = 'combo' " +
                            ...
                            " left outer join salesmodifiers on salesitem.id = salesmodifiers.salesitemid  group by salesitem.id ) as innercombo on combo.id = innercombo.comboid " +
                            ...
                            " where sales.status = 'Closed' and DATE(sales.saledate) between   '" + startdate.ToString("yyyy-MM-dd") + "' and '" + enddate.ToString("yyyy-MM-dd") + "'   group by  salesitem.reportcategory) as netsales " +
                            " on reportsetup.reportcategory = netsales.reportcategory where active=1 and grouptype = 'RevenueCategory' group by reportgroup.reportgroup order by sortorder";
        ...
        {
            string query = "insert into reportgroup (grouptype,reportgroup,sortorder,active) values ('" + grouptype + "','" + reportgroup + "',(select max(sortorder) from reportgroup as max) + 1, 1)";
        ...
        public decimal GetTipsWithHeld(DateTime startdate, DateTime enddate)
		{
			...
			string query = "select sum( payment.tipamount) as gratuity from sales inner join payment on sales.id = payment.salesid where sales.status = 'Closed' and DATE(sales.saledate) between '" + startdate.ToString("yyyy-MM-dd") + "' and '" + enddate.ToString("yyyy-MM-dd") + "'";
			dt = m_dbconnect.GetData(query);
