    ...
    public class DBProducts
    {
        ...
        public DataTable GetAllProducts(string orderby)
        {
           ...
           // query = "select * from  product order by SUBSTRING(product.menuprefix,1,1), cast(SUBSTRING(product.menuprefix,2) as unsigned), product.modelnumber, product.id ";
             query = "select 'all items' as category, product.*, concat('pack://siteoforigin:,,,/',product.imagesrc) as imgurl from  product  " + orderby;
        ...
        public DataTable GetRecentProducts(string orderby)
        {
            ...
            query = "select   product.* from  product inner join (select distinct productid from salesitem order by id desc limit 10) as list on product.id = list.productid  " + orderby;
        ...
        {
            string query = "select printers.description as printer, category.description as category, category.colorcode as catcolorcode, product.*,concat('pack://siteoforigin:,,,/',product.imagesrc) as imgurl from category " + 
                ...
                " where category.id =" + catid + " order by SUBSTRING(product.menuprefix,1,1), cast(SUBSTRING(product.menuprefix,2) as unsigned), product.id";
      ...
      " select  menuprefix,concat(description,'(clone)'),modprofileid,price,imagesrc,type,taxexempt,commissiontype,colorcode,reportcategory,cost,locationcode,commissionamt from product where id=" + id;
        ...
        public DataTable GetModGroupModifiers(int modgroupid)
        {
            ...
            string query;
            query = "select * from modifiers inner join modgroup2modifier on modifiers.id = modgroup2modifier.modifierid where modgroup2modifier.modgroupid = " + modgroupid;
            return m_dbconnect.GetData(query);
        ...
        {
            string query;
            query = "select * from modifiers where id not in (select modifierid from modgroup2modifier where modgroup2modifier.modgroupid =" + modgroupid + ")";
