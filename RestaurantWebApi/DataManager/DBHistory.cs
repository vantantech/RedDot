...
namespace RedDot
{
    public class DBHistory
    {
        ...
        public DataTable GetClosedVoidedOrders(DateTime startdate, DateTime enddate, int employeeid, string transtype)
        {
            ...
            string query = "Select 0 as selected, sales.id, sales.ordernumber ,if(pay.auth > 0,'AUTH','OTHER') as transtype, customer.phone1,  sales.saledate, TIME(sales.saledate) as time,sales.status, pay.tip, sales.total, (sales.total - coalesce(pay.netamount,0)) as balance, pay.paymenttype, employee.displayname from sales" +
                        ...
                        " left outer join (select salesid,sum(if(void=0,netamount,0)) as netamount,sum(tipamount) as tip, group_concat(concat(cardgroup ,'-', transtype) ) as paymenttype,sum(if((transtype='AUTH' && void=0),1,0)) as auth from payment group by salesid ) as pay on sales.id = pay.salesid " +
