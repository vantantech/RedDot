...
{
    public class PAXVM:CCPBaseVM
    {
        private TriPOSModel triposmodel = new TriPOSModel(GlobalSettings.Instance.LaneId);
        private PAXModel paxmodel = new PAXModel(GlobalSettings.Instance.LaneId);

        public PAXVM(Window parent, Ticket m_ticket, string transtype, Payment payment)
            ...
            try
            {
                ...
                if (temp != "")
                {
                  ...
                  //  SaleResponse resp = triposmodel.ExecuteCreditSale(CurrentTicket.SalesID, amt, referencenumber);
                    PaymentResponse resp = paxmodel.ExecuteCreditSale(CurrentTicket.SalesID, amt, referencenumber);
                    ...
                    if (resp != null)
                    {
                        switch (resp.ResultCode.ToUpper())
                        {
                            ...
                            case "000000": //OK
                                ...
                                Payment payment = m_salesmodel.GetPayment(resp.RefNum);
                            ...
                            default:
                                ...
                                TouchMessageBox.Show(resp.ResultCode + ":" + resp.ResultTxt);
                                break;
    ...
    */
        ...
        private bool AddCreditPayment(string transtype, decimal requested_amount, PaymentResponse response, DateTime paymentdate)
        {
            if (CurrentTicket == null)
                return SalesModel.InsertCreditPayment(transtype, 0, requested_amount, response, paymentdate);
            else
            {
                var result = SalesModel.InsertCreditPayment(transtype, CurrentTicket.SalesID, requested_amount, response, paymentdate);
