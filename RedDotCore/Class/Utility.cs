...
namespace RedDot
{
    public class Utility
    {
        ...
        public static string GetINIString(string key, string section)
        {
            var MyIni = new IniFile("RedDot.ini");
            return MyIni.Read(key, section);

        }

        public static void PutINIString(string key, string val, string section)
        {
            var MyIni = new IniFile("RedDot.ini");
        ...
        public static int RandomPin(int digits)
        {
            ...
            int start = (int)Math.Pow(10, digits-1); //minimum range is based on digit length required
            int stop = 2000000000;  //two billion as top
            string rand = random.Next(start,stop ).ToString();
            return int.Parse(rand.Substring(rand.Length - digits));
        }
