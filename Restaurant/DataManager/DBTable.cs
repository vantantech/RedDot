...
namespace RedDot
{
    public class DBTable
    {
       ...
       public DataTable GetMaxTable()
       {
        ...
        public bool SaveTable(int id,int left, int top, int width, int height, int number,int seats, string color)
       {
           string query;

           query = "update tablelayout set tableleft=" + left + ",tabletop=" + top + ",width=" + width + ",height=" + height + ",number=" + number + ",seats=" + seats + ",color='" + color + "' where id=" + id;
           return _dbconnect.Command(query);

       }

        public bool SaveAreaImage(int areaid,string imagesrc)
        {
            string query = "update area set backgroundimage ='" + imagesrc + "' where id=" + areaid;
            return _dbconnect.Command(query);
        }
